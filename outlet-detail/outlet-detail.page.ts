...
export class OutletDetailPage extends PageBase {
    ...
    checkPhoneNumber(){
        if (this.formGroup.controls.WorkPhone.valid) {
            this.pageProvider.search({WorkPhone_eq: this.formGroup.controls.WorkPhone.value}).toPromise().then((result:any)=>{
                if (result.length == 0 && result.findIndex(e=>e.Id==this.id)) {
                    ...
                    this.formGroup.controls.WorkPhone.setErrors({'incorrect': true});
                }
