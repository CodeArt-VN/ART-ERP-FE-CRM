...
export class BusinessPartnerPage extends PageBase {
	...
	submitBusinessPartner() {
		this.alertCtrl
			.create({
				header: 'Gửi duyệt',
				//subHeader: '---',
				message: 'Sau khi gửi duyệt, bạn không thể chỉnh sửa đối tượng được nữa. Bạn có chắc muốn gửi duyệt tất cả đối tượng chưa duyệt?',
				buttons: [
					{
						text: 'Không',
						role: 'cancel',
	...
	approveBusinessPartner() {
		...
		if (itemsCanNotProcess.length == this.selectedItems.length) {
			this.env.showMessage('Selected Business Partners Cannot Be Approved. Submitted Business Partners Only!', 'warning');
		} else {
	...
	disapproveBusinessPartner() {
		...
		if (itemsCanNotProcess.length == this.selectedItems.length) {
			this.env.showMessage('Selected Business Partners Cannot Be Disapproved. Submitted Business Partners Only!', 'warning');
		} else {
			itemsCanNotProcess.forEach((i) => {
				i.checked = false;
			});
